...
% xoil=[ 0.2214    0.0703    0.2933    0.4149];
...
while(absdKi>=E);
    ...
    F1 = sum(zi.*(1-Ki)./(Ki+(1-Ki).*lmin));
    F2 = sum(zi.*(1-Ki)./(Ki+(1-Ki).*lmax));
...
% fprintf('xi = %d',yi);
% fprintf('\n');
% fprintf('Z liquid = %d',ZL);
% fprintf('\n');
% fprintf('Z vapor = %d',ZV);
% fprintf('\n');
% fprintf('\n');
